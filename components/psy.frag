// from https://www.shadertoy.com/view/Md2GRc by gleurop
#ifdef GL_ES
...
vec3 formula(in vec2 p, in vec2 c)
{
	const float n = 2.0;
	const int iters = 12;
	float time2 = time*(0.01*mouse.y*0.01);
	vec3 col = vec3(0);
	float t = 1.0;
	float dpp = dot(p, p);
	float lp = sqrt(dpp);
	...
	for (int i = 0; i < iters; i++) {
		...
		* smoothstep(0.0, 0.01, abs(p.y)*2.0);
		...
		col += hsv(1.0*inputval1 - max(p.x, p.y) + t*2.0 + time2, 2.0-lp+t, r) ;
	}
...
void main(){
	...
	vec2 c = time*e;
//c = 8.0*iMouse.xy/resolution.xy;
	float d = 1.0;
	vec3 col = vec3(0.0);
	const float blursamples = 1.0;
